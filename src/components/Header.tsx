...
export default function Header() {
  ...
  return (
    <header className="w-full relative z-50">
      <div className={`w-full max-w-[1100px] mx-auto px-6 py-8 ${isOpen ? 'bg-[#050505]/70 backdrop-blur-xl' : ''} flex items-center justify-between`}>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#050505]/70 backdrop-blur-xl border-b border-white/10 py-6 px-6 flex flex-col gap-4 text-[15px] font-medium text-zinc-200 shadow-2xl">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="py-2 hover:text-white text-[#4b6ffe] font-semibold"
            onClick={() => setIsOpen(false)}
          >
            Support Us (Donate)
          </a>
